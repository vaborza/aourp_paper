# Default experiment set - simulates recruitment using EHR sites
...
for i in range(6):
    if i < 5:
        quarterly_cumu_cohorts.append(pd.read_pickle(f'~/data/ehr_sites/aou_quarterly_cumu_cohorts_{4 * i + 3}.pkl'))
        quarterly_indiv_cohorts.append(
            pd.concat([
                pd.read_pickle(f'~/data/ehr_sites/aou_quarterly_indiv_cohorts_{4 * i}.pkl'),
                pd.read_pickle(f'~/data/ehr_sites/aou_quarterly_indiv_cohorts_{4 * i + 1}.pkl'),
                pd.read_pickle(f'~/data/ehr_sites/aou_quarterly_indiv_cohorts_{4 * i + 2}.pkl'),
                pd.read_pickle(f'~/data/ehr_sites/aou_quarterly_indiv_cohorts_{4 * i + 3}.pkl'),
            ]
            ).groupby('SITE').sum().reset_index()
        )
    else:
        quarterly_cumu_cohorts.append(pd.read_pickle(f'~/data/ehr_sites/aou_quarterly_cumu_cohorts_{4 * i}.pkl'))
        quarterly_indiv_cohorts.append(pd.read_pickle(f'~/data/ehr_sites/aou_quarterly_indiv_cohorts_{4 * i}.pkl'))

def se_y(x, se_radius):
    n = 0.522
    return (KLD_CU * se_radius) * (1 - (x / (KLD_UC * se_radius)) ** n) ** (1 / n)

# Functions necessary for optimization
...
def objective_from_strategy(resource_vector, current_cohort, site_expected_demographics, objective_type, target_point):
    ...
    expected_recruitments = np.matmul(resource_vector_prime, site_expected_demographics) # Expected recruitments for the given resource vector
...
def optimize_recruitment_strategy(num_recruitments, current_cohort, site_categoricals, objective='dist_opt', upper_bound_factor=5, target_point=None):
    ...
    optimal_strat = minimize(fun=objective_from_strategy,
                             x0=np.ones(shape=len(site_categoricals)), # Start with an even distribution of resources to all sites
                             args=(current_cohort, site_expected_demographics, objective, target_point), # Other inputs to find the objective function
                             method='Powell', # Powell's method is a fast an effective optimizer for what we need.
                             options= {'xtol':1 / num_recruitments, 'disp':False}, # Limit the resource vector search space to no less than 1 whole recruitment because fractional recruitments would not be possible
                             bounds=[(0, upper_bound_factor)] * len(site_categoricals), # Bound the softmax allocation factors between 0 and a hyperparameter, to prevent full resource allocation to 1 site (which may not be realistic)
                             ).x
    attempted_recruitments = softmax(optimal_strat) * num_recruitments # Turn the optimized resource vector into an actual recruitment number
    actual_recruitments = np.floor(attempted_recruitments).astype(int) # Round down recruitments to nearest whole numbers
    remainder_recruitments = np.round(np.sum(attempted_recruitments - actual_recruitments)).astype(int) # Find the number of leftover recruitments
...
def simulate_recruitment(args):
    ...
    site_dirichlets = pd.DataFrame.from_dict({'SITE':site_dirichlet_dict.keys(), 'DIRICHLET':site_dirichlet_dict.values()})
    ...
    for i in range(len(quarterly_indiv_cohorts) - starting_steps):
        ...
        sites_without_samples = site_dirichlets['DIRICHLET'][dirichlets_available_sites].apply(lambda row: row.sum()) == 0
        ...
        if site_prior == 'census':
            ...
            for new_mz3 in site_dirichlets['SITE'][dirichlets_available_sites][sites_without_samples]:
                mz3_orig_demo = quarterly_indiv_cohorts[i]['DEMOGRAPHICS'][quarterly_indiv_cohorts[i]['SITE'] == new_mz3].to_numpy()
        ...
        if prior_update == 'sim':
            ...
            for existing_mz3 in site_dirichlets['SITE'][dirichlets_available_sites][~sites_without_samples]:
                site_dirichlets.loc[site_dirichlets['SITE'] == existing_mz3, 'DIRICHLET'] = [site_knowledge_dirichlets['DIRICHLET'].loc[site_knowledge_dirichlets['SITE'] == existing_mz3].item()]
        elif prior_update == 'actual':
            #  Update site priors with the actual AoURP recruitment values
            for existing_mz3 in site_dirichlets['SITE'][dirichlets_available_sites][~sites_without_samples]:
                mz3_recruited = quarterly_cumu_cohorts[i-1]['DEMOGRAPHICS'][quarterly_cumu_cohorts[i-1]['SITE'] == existing_mz3].to_numpy()
        ...
        estimated_demographics = site_dirichlets['DIRICHLET'][dirichlets_available_sites].apply(lambda row: rng.dirichlet(row + 1e-10))
        ...
        dirichlets_to_cumulative_mapping = site_dirichlets[dirichlets_available_sites].merge(quarterly_cumu_cohorts[i], on='SITE', how='left')

        # Find the optimal recruitment strategy
        recruits = optimize_recruitment_strategy(num_recruitments, cohort, estimated_demographics, objective, upper_bound_factor, target_point)
        ...
        for site_id, num_recruits in enumerate(recruits):
            site_demographics = dirichlets_to_cumulative_mapping['DEMOGRAPHICS'].iloc[site_id] # For each available site, find its demographics
            site_mz3 = dirichlets_to_cumulative_mapping['SITE'].iloc[site_id] # Determine the matching location for that site ID
            recruitments_record[site_mz3] = num_recruits # Add to the recruitment record the number of desired recruitments
            site_distribution = site_demographics / np.sum(site_demographics) # Generate a categorical distribution for the site response distribution
            draws = rng.choice(60, size=num_recruits, replace=True, p=site_distribution) # Recruit from this respone distribution
            ...
            if prior_update == 'sim':
                ...
                site_knowledge_dirichlets.loc[site_knowledge_dirichlets['SITE'] == site_mz3, 'DIRICHLET'].iat[0] += site_recruited_cohort
...
num_trials = 40 # Number of experimental replicates
...
parameters = list(product([prior], [update_method], [objective], range(num_trials), [upper_bound_factor], [target_point], [starting_cohort], [starting_steps]))
...
with open(f'~/paper_results/ehr_site_{prior}_prior_{update_method}_update_{objective}_objective_{upper_bound_factor}_upper_bound_factor_004_0909_yearly.pickle', 'wb') as handle:
