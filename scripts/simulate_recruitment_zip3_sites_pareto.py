# Defines the Pareto frontier for aggregated ZIP3 site recruitment
...
# AoURP and Census data by generalized (i.e., modified) ZIP3 values
# aou_census_imputed = pd.read_pickle('~/data/aou_census_imputed_generalized.pkl')
...
for i in range(21):
    ...
    quarterly_indiv_cohorts.append(pd.read_pickle(f'~/data/aou_quarterly_indiv_cohorts_{i}.pkl'))
...
def simulate_recruitment(args):
    ...
    site_dirichlet_dict = {}
    for mz3 in quarterly_cumu_cohorts[-1]['MOD_ZIP3']:
        site_dirichlet_dict[mz3] = np.zeros(60)  # There are 60 demographic groups
    site_dirichlets = pd.DataFrame.from_dict(
        {'MOD_ZIP3': site_dirichlet_dict.keys(), 'DIRICHLET': site_dirichlet_dict.values()})
    ...
    for i in range(len(quarterly_indiv_cohorts) - starting_steps):
        ...
        sites_available_to_sample = quarterly_cumu_cohorts[i]['MOD_ZIP3'].to_numpy()
        dirichlets_available_sites = site_dirichlets['MOD_ZIP3'].isin(sites_available_to_sample)
        ...
        if site_prior == 'census':
            ...
            for new_mz3 in site_dirichlets['MOD_ZIP3'][dirichlets_available_sites][sites_without_samples]:
                site_dirichlets.loc[site_dirichlets['MOD_ZIP3'] == new_mz3, 'DIRICHLET'] = [
                    aou_census_imputed.loc[new_mz3]]
            ...
            for new_mz3 in site_dirichlets['MOD_ZIP3'][dirichlets_available_sites][sites_without_samples]:
                mz3_orig_demo = quarterly_indiv_cohorts[i]['DEMOGRAPHICS'][
                    quarterly_indiv_cohorts[i]['MOD_ZIP3'] == new_mz3].to_numpy()
                ...
                site_dirichlets.loc[site_dirichlets['MOD_ZIP3'] == new_mz3, 'DIRICHLET'] = [mz3_demo[0]]
        elif site_prior == 'jeffreys':
            # Initialize new sites to an uninformative Jeffreys prior
            for new_mz3 in site_dirichlets['MOD_ZIP3'][dirichlets_available_sites][sites_without_samples]:
                site_dirichlets.loc[site_dirichlets['MOD_ZIP3'] == new_mz3, 'DIRICHLET'] = [0.5 * np.ones(60)]

        if prior_update == 'sim':
            # Update the priors using recruited individuals in simulation
            for existing_mz3 in site_dirichlets['MOD_ZIP3'][dirichlets_available_sites][~sites_without_samples]:
                site_dirichlets.loc[site_dirichlets['MOD_ZIP3'] == existing_mz3, 'DIRICHLET'] = [
                    site_knowledge_dirichlets['DIRICHLET'].loc[
                        site_knowledge_dirichlets['MOD_ZIP3'] == existing_mz3].item()]
            ...
            for existing_mz3 in site_dirichlets['MOD_ZIP3'][dirichlets_available_sites][~sites_without_samples]:
                mz3_recruited = quarterly_cumu_cohorts[i - 1]['DEMOGRAPHICS'][
                    quarterly_cumu_cohorts[i - 1]['MOD_ZIP3'] == existing_mz3].to_numpy()
                site_dirichlets.loc[site_dirichlets['MOD_ZIP3'] == existing_mz3, 'DIRICHLET'] = [mz3_recruited[0]]
        ...
        #  Simulate the recruitment via random draw from the site's cumualtive demographics distribution
        for site_id, num_recruits in enumerate(recruits):
            ...
            site_mz3 = dirichlets_to_cumulative_mapping['MOD_ZIP3'].iloc[
                site_id]  # Determine the matching location for that site ID
            recruitments_record[
                site_mz3] = num_recruits  # Add to the recruitemnt record the number of desired recruitments
            ...
            if prior_update == 'sim':
                ...
                site_knowledge_dirichlets.loc[site_knowledge_dirichlets['MOD_ZIP3'] == site_mz3, 'DIRICHLET'].iat[
...
upper_kld_c = 0.2
target_points = [(x * upper_kld_c / num_trials, se_y(x * upper_kld_c / num_trials, 1)) for x in range(num_trials)]
starting_cohort = None
starting_steps = 0

parameters = list(product([prior],
                          [update_method],
                          [objective],
                          [8080],
                          [upper_bound_factor],
                          (target_points[i] for i in range(num_trials)),
                          [starting_cohort],
                          [starting_steps]))
...
with open(
        f'~/paper_results/zip3_site_{prior}_prior_{update_method}_update_{objective}_objective_{upper_bound_factor}_upper_bound_factor_pareto.pickle',
        'wb') as handle:
